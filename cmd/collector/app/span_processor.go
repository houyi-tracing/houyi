...
type spanProcessor struct {
	...
	metrics            *app.SpanProcessorMetrics
	preProcessSpans    app.ProcessSpans
	filterSpan         app.FilterSpan         // filter is called before the sanitizer but after preProcessSpans
	sanitizer          sanitizer.SanitizeSpan // sanitizer is called before processSpan
	processSpan        app.ProcessSpan
	logger             *zap.Logger
...
func newSpanProcessor(spanWriter spanstore.Writer, opts ...Option) *spanProcessor {
	...
	processSpanFuncs := []app.ProcessSpan{options.preSave, sp.saveSpan}
	...
	sp.processSpan = app.ChainedProcessSpan(processSpanFuncs...)
	return &sp
}
