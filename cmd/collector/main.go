...
const serviceName = "houyi-collector"
...
func main() {
	...
	strategyStoreFactory := sampling.NewFactory()
	filterFactory := filter.NewFilterFactory()

	v := viper.New()
	command := &cobra.Command{
		Use:   "houyi-collector",
		Short: "Houyi collector receives and processes traces from Houyi agents and clients",
		Long:  `Houyi collector receives traces from Houyi agents and runs them through a processing pipeline.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			filterFactory.Initialize(v, logger)
			spanFilter := filterFactory.CreateFilter()
