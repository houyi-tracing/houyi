...
import (
	"fmt"
	jaeger "github.com/jaegertracing/jaeger/cmd/agent/app"
	"os"
...
func main() {
	...
	var command = &cobra.Command{
		Use:   "houyi-agent",
		Short: "Houyi agent is a local daemon program which collects tracing data.",
		Long:  `Houyi agent is a daemon program that runs on every host and receives tracing data submitted by Houyi client libraries.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			baseFactory := svc.MetricsFactory.
				Namespace(metrics.NSOptions{Name: "houyi"}).
			...
			builders := map[reporter.Type]jaeger.CollectorProxyBuilder{
				reporter.GRPC: jaeger.GRPCCollectorProxyBuilder(grpcBuilder),
			}
			cp, err := jaeger.CreateCollectorProxy(jaeger.ProxyBuilderOptions{
				Options: *rOpts,
				Logger:  logger,
				Metrics: mFactory,
			}, builders)
			...
			if err != nil {
				return fmt.Errorf("unable to initialize Houyi Agent: %w", err)
			}
